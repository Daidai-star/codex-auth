...
test "history sync mirrors custom and openai threads exactly once" {
...
test "history sync does not duplicate an existing mirrored thread after source timestamps change" {
    const gpa = std.testing.allocator;
    if (!sqlite3Available(gpa)) return error.SkipZigTest;
    ...
    try std.testing.expectEqual(@as(usize, 0), second_summary.mirrored_threads);
